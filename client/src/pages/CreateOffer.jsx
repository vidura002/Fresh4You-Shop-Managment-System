...
const CreateOffer = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const OfferData = { offerID, name, price, variant, quantity, description, image };
    try {
      const response = await axios.post('http://localhost:3000/api/Offer/create', OfferData);
      ...
      if (error.response && error.response.status === 400 && error.response.data.error === 'Offer ID already exists') {
        setAlertMessage('Offer ID already exists. Please enter a different Offer ID.');
      } else {
    ...
    <div className="mt-10 ml-20 mr-20 p-10 bg-green-200 shadow-md rounded-md">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div> <div className="mb-4">
          <label htmlFor="offerID" className="block text-gray-700 font-bold mb-2">
            Offer ID
          </label>
          <input
            type="text"
